...
bool Point::operator< (Point const& arg2) const {
	return ((x*x + y*y) < (arg2.x * arg2.x + arg2.y * arg2.y));
}
...
std::pair<bool, bool> get_location(
	PointType const& p1x, //check which side of the line (p1x,p1y)-->(p2x,p2y) 
	PointType const& p1y, //point (qx,qy) is on
	PointType const& p2x,
	PointType const& p2y,
	PointType const& qx,
	PointType const& qy
	)
{
	...
	PointType scal_prod = norm.x*point.x + norm.y*point.y;
...
bool checkPoints(unsigned int point1Index, unsigned int point2Index, std::vector< Point > const& points) {
	...
	PointType xDiff = point2.x - point1.x;
	PointType yDiff = point2.y - point1.y;
	...
	for (unsigned int i = 0; i < points.size(); ++i) {
		...
		PointType lineEquationResult = (pointToCheck.y - point1.y) * xDiff - (pointToCheck.x - point1.x) * yDiff;
...
void findHullVertexRec(std::vector<int> & hullIndices, std::vector< Point > const& points) {
	...
	bool sameSide = true;
	int lastVertexX = points[lastVertex].x;
	int lastVertexY = points[lastVertex].y;
	int possibleVertexX, possibleVertexY;
	...
	for (int i = pointsSize - 1; i >= 0; --i) {
		...
		for (int k = 0; k < pointsSize && sameSide; ++k) {
			if (k == i || k == lastVertex)
				continue;
			std::pair<bool, bool> locationResult = get_location(lastVertexX, lastVertexY, possibleVertexX, possibleVertexY, points[k].x, points[k].y);
			sameSide = sameSide && (!locationResult.second);
		}
